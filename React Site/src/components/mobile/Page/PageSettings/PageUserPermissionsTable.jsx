...
function PageUserPermissionsTable({
  ...
  async function deletePagePermission(userPermission) {
    ...
    try {
      const fetchDeletePagePermission = await fetch(
        APIAdress + `page-permissions/page-permission`,
        settings
      );

      if (
        !fetchDeletePagePermission.ok &&
        fetchDeletePagePermission.status !== 400
      ) {
        throw new Error('Cannot Connect to Server');
      }

      const response = await fetchDeletePagePermission.json();
      if (response.success == true) {
        const users = response.data;
        getAllPagePermissions();
        getAllUsersWithPagePermissions();
    ...
    if (viewTableorAddorUpdate == 0) {
      return (
        <>
          <div className="mobile-settings-section">
            <br></br>
            <h3 className="mobile-section-title">Page Permissions</h3>
            <div className="mobile-table-window">
              ...
              {usersPagePermissions.length > 0 ? (
                <>
                  <table className="mobile-permissions-table">
                    <thead>
                      <tr>
                        <th>Username</th>
                        <th>Permission</th>
                    ...
                    <tbody>
                      ...
                      {usersPagePermissions.map((userPermission) => (
                        <tr key={userPermission.id}>
                          ...
                          <td>
                            <button
                              className="mobile-settings-del2"
                              onClick={() =>
      ...
      return (
        <>
          <div className="mobile-settings-section">
            <br></br>
            <h3 className="mobile-section-title">New Page Permission</h3>
            <br></br>
      ...
      return (
        <>
          <div className="mobile-settings-section">
            <br></br>
            <h3 className="mobile-section-title">Update Page Permission</h3>
            <br></br>
            <div className="mobile-edit-window">
              <p className="mobile-settings-error">{error}</p>
              <div className="mobile-edit-page-details">
